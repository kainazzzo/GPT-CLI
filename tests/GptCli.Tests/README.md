...
- Core: `GptOptions` secret JSON ignore, `ParameterMapping` request mapping, GPT-6 sampling/tool compatibility, embed file/directory IO, async enumerable helpers.
- OpenAI I/O: fake `IOpenAIService` chat/embeddings, stub HTTP Responses API mapping, token counting.
